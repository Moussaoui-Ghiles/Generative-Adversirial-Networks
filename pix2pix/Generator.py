...
class Block(nn.Module):
    def __init__(self, in_chan, out_chan, down = True, act = "relu", use_dropout = False):
        super().__init__()
...
class Generator(nn.Module):
    def __init__(self, in_chan=3, features = 64):
        ...
        self.final_up = nn.Sequential(
            nn.ConvTranspose2d(features * 2, in_chan, kernel_size=4, stride=2, padding=1),
            nn.Tanh(),
        )
    def forward(self, x):
...
def test():
    ...
    model = Generator(in_chan=3, features=64)
    preds = model(x)
    print(preds.shape)
